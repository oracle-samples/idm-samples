    ...
    try {
      ...
      xhr.addEventListener("readystatechange", function () {
        ...
        if (this.readyState == 4) {
          ...
          if ((jsonData.authFactor === 'USERNAME_PASSWORD' || jsonData.authFactor === 'USERNAME') && this.status == 401 && this.getResponseHeader("Content-type") === 'text/html') {
              ...
              if (jsonData["authFactor"] !== "USERNAME") {
                ...
                if (jsonData.op === "getBackupFactors") {//condition to use query selector and add buttons
                  ...
                  if (jsonResponse.nextAuthFactors.length === 1) {
                    subdiv.innerHTML += '<div class="sameline"><span class="info" data-res="totp-info-msg">No alternative factors are configured for this user.</span><span class="device-name"></span></div>';
