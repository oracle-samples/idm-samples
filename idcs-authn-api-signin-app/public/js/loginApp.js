function LoginApp() {
  ...
  this.buildFirstForm = function (formDiv, showUidPw, IDPdata) {
    ...
    if (showUidPw) {
      ...
      if (!userNameFirst) {
        password = '<label><span data-res="signin-password-fld">Password</span><input type="password" id="password" value="" required></label>'
      }
      formDiv.innerHTML +=
        '<label><span data-res="signin-username-fld">Username</span><input type="text" id="userid" value="" required></label>' + password
        +
    ...
    if (showUidPw && userNameFirst) {
      this.handleClickToSubmitEvent(formDiv, this, 'postUserNameFirst');
      this.handleKeyPressToSubmitEvent(formDiv, formDiv.querySelector("#userid"), this, 'postUserNameFirst');
      this.handleClickEvent(formDiv, this);
    }
  ...
  //Displays the form for UserNameFirst flow
  ...
  this.displayAltFactorsSubform = function (payload) {
    ...
    this.replaceDiv("signin-div", otherFactorsDiv, false);
  };
  ...
  this.initiateAlternativeFactor = function (factor, payload, device) {
    ...
    switch (factor) {
      ...
      case 'FIDO_AUTHENTICATOR':
        this.sdk.initAuthnFido({ "deviceId": device.deviceId });
        break;
      default:
